...
class URI{
	...
	public function __construct(&$pathsettings){
		...
		// echo $this->_permitted_uri_chars;
		if(is_cli()){
	...
	protected function _parse_uri($str, $cli=FALSE){
		if($cli){
			...
			foreach (explode('/', trim(parse_url($this->uri_string)["path"], '/')) as $segment)
			{
	...
	{
		// echo preg_match('/^['.$this->_permitted_uri_chars.']+$/i'.'u', $str) ? "yes" : "no";
		// echo $str;
